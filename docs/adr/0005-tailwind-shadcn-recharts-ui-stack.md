...
Vendored UI primitives in `apps/web-app` use [**Base UI**](https://base-ui.com/react/) (`@base-ui/react`) for headless behavior: **Button** (native submit/focus semantics), **Dialog** (filter panel), and plain **`<label>`** for labels. We previously used Radix (`@radix-ui/react-slot`, `@radix-ui/react-dialog`, `@radix-ui/react-label`); Radix was removed in favor of a single primitives dependency alongside other Base UI surfaces (e.g. combobox elsewhere).
